...
mod async_pubsub {
    ...
    macro_rules! impl_sink_func_for {
        ($name: ident, $type: ident, $res: ty) => {
            #[allow(dead_code)]
            async fn $name(&mut self, arg: impl ToRedisArgs) -> RedisResult<$res> {
                match self {
    ...
    macro_rules! impl_sink_func {
        ($name: ident, $res: ty) => {
            impl Wrapper {
                impl_sink_func_for!($name, Wrapper, $res);
            }
            impl WrapperSink {
                impl_sink_func_for!($name, WrapperSink, $res);
            }
        };
    }
    ...
    impl_sink_func!(ping_message, impl FromRedisValue);
    ...
    impl Wrapper {
        async fn ping<T: FromRedisValue>(&mut self) -> RedisResult<T> {
            match self {
                Wrapper::PubSub(pubsub) => pubsub.ping().await,
                Wrapper::Manager(pubsub_manager) => pubsub_manager.ping().await,
            }
        }
        ...
        test_with_all_pubsub_types(
            |ctx, pubsub_conn| async move {
                ...
                if ctx.protocol == ProtocolVersion::RESP3 {
                    ...
                    let message: Vec<String> = sink.ping_message("foobar").await?s;
    ...
    fn can_receive_messages_from_split_pubsub_after_sink_was_dropped(
        ...
        test_with_all_pubsub_types(
            |ctx, pubsub_conn| async move {
                ...
                for _ in 0..repeats {
                    let message: String = stream.next().await?.get_payload()?;
